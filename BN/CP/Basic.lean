...
lemma posSemidef_vecMulVec_self (p : n → ℝ) : (vecMulVec p p).PosSemidef :=
  .of_dotProduct_mulVec_nonneg (by
    rw [isHermitian_iff_isSymm]
    exact transpose_vecMulVec p p) fun x ↦ by
...
omit [Fintype n] [DecidableEq n] in
lemma IsCompletelyPositive.submatrix {ι : Type*} [Fintype ι] {C : Matrix n n ℝ}
