...
lemma schurComplementEntry_eq_minor_div_det (A : Matrix ι ι ℝ) (e : Fin r → ι)
  ...
  letI : Invertible AII := hunit.invertible
...
lemma schurComplementEntry_diag_pos_of_posDef {n : Type*} [Fintype n] [DecidableEq n]
...
lemma det_submatrix_finset_eq {n : Type*} [Fintype n] [DecidableEq n]
...
lemma det_submatrix_subtype_eq_orderEmb {n : Type*}
    [Fintype n] [LinearOrder n] [DecidableEq n]
    (A : Matrix n n ℝ) (S : Finset n) :
    (A.submatrix (Subtype.val : {x // x ∈ S} → n) Subtype.val).det =
...
lemma det_MX_snoc_z_nonneg (s t : Fin k → ℝ) (ρ x : Fin p → ℝ)
  ...
  have hG :
    ...
    ext a b
    simp [hlast, Matrix.add_apply, diagonal_apply]
  simp_rw [hG]
...
lemma elimTE_eq_conjTranspose (s t : Fin k → ℝ) (ρ x : Fin p → ℝ) :
  ...
  ext i j
  simp [elimTE, elimET, conjTranspose_apply]
...
lemma schurComplementEntry_submatrix {n : Type*} [Fintype n] [DecidableEq n]
...
lemma schurComplementEntry_eq_zero_of_mem {n : Type*} [Fintype n] [DecidableEq n]
...
lemma elimGramE_eq_z0_add_Z (s t : Fin k → ℝ) (ρ x : Fin p → ℝ)
  ...
  simp only [elimGramE, elimColE, elimGramZLead, elimColZLead, elimZLead]
  rw [hinv, elimEEInv_mulVec]
  simp [dotProduct, Fintype.sum_option, elimEEmbed]
  rfl
...
lemma gram_inv_castSucc_step {m : ℕ}
  ...
  have hxlast : x (Fin.last m) = resP Pβ / d := by
    ...
    have hddef : d =
      ...
      simp [d, schurComplementEntry, A, b, hrow, submatrix_apply]
...
lemma elimZLead_castSucc (s t : Fin k → ℝ) (ρ x : Fin p → ℝ)
  ...
  ext i j
  simp [elimZLead, submatrix_apply, Fin.ext_iff]
...
lemma elimC0_eq_sum_scaled (s t : Fin k → ℝ) (ρ x : Fin p → ℝ)
  ...
  simp [elimC0Factor, elimVec0, elimVec]
  congr 1
...
lemma elimC0_eq_elimGramE (s t : Fin k → ℝ) (ρ x : Fin p → ℝ)
  ...
  simp [Matrix.add_apply, Matrix.smul_apply, Matrix.sum_apply, vecMulVec_apply,
    mul_left_comm, inv_mul_cancel₀ h00]
...
lemma elimGramE_y_embed (s t : Fin k → ℝ) (ρ x : Fin p → ℝ)
  ...
  have hcol : elimColE s t ρ x (elimEEmbed b) = fun i => elimEE s t ρ x i b := by
    funext i
    simp [elimColE, elimEE, submatrix_apply]
    exact (MX_isSymm s t ρ x).apply _ _
  rw [hrow, hcol]
  have hcolb :
...
lemma mul_mul_col_apply {l m n o : Type*} [Fintype m] [Fintype n]
  ...
  simp [Matrix.mul_apply, dotProduct, mulVec, Finset.mul_sum, Finset.sum_mul, mul_assoc]
  exact Finset.sum_comm

lemma elimGramE_y_y (s t : Fin k → ℝ) (ρ x : Fin p → ℝ)
    (hs : ∀ i, 0 < s i) (ht : ∀ i, 0 < t i) (hρ : ∀ j, 0 < ρ j)
    (hx : ∀ j, 0 ≤ x j) (j ℓ : Fin p) :
...
lemma M_eq_elimGramE_add_extend (s t : Fin k → ℝ) (ρ x : Fin p → ℝ)
  ...
  · intro j
    ...
    · intro ℓ
      ...
      rw [extendByZeroT_y_y, elimGramE_y_y s t ρ x hs ht hρ hx]
