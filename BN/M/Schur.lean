...
import BN.CP.Basic
import BN.Kernel.Data
import BN.Kernel.Main
import BN.M.Config
import BN.M.Elim
...
lemma schurComplement_woodbury {ε τ : Type*}
    [Fintype ε] [DecidableEq ε] [Fintype τ] [DecidableEq τ]
...
lemma e0_dot_e0 : e (0 : Fin 3) ⬝ᵥ e (0 : Fin 3) = 1 := by
  simp [e, dotProduct, Fin.sum_univ_three, Pi.single_apply]
...
lemma configL_z0_z (i : Fin k) : configL s t ρ x idxZ0 (idxZ i) = -s i := by
  ...
  have h1 : ∑ i', s i' *
    ...
    · intro i' hi'
      ...
      rw [edge_z0]; simp [hne, idxZ0_ne_idxZ (p := p) i]
  have h2 : ∑ i', ∑ j, (s i' * ρ j * truncSq (t i') (x j)) *
      vecMulVec (e (idxZ (p := p) i') - e (idxY (k := k) j))
...
lemma configL_z0_y (j : Fin p) : configL s t ρ x idxZ0 (idxY j) = 0 := by
  ...
  have h1 : ∑ i, s i *
    ...
    rw [edge_z0]
    simp [idxZ0_ne_idxY (k := k) j, idxZ_ne_idxY (p := p) i j]
  have h2 : ∑ i, ∑ j', (s i * ρ j' * truncSq (t i) (x j')) *
      vecMulVec (e (idxZ (p := p) i) - e (idxY (k := k) j'))
...
lemma configL_z_z (i h : Fin k) :
  ...
  have h1 : ∑ i', s i' *
    ...
    by_cases hih : i = h
    · subst hih
      rw [if_pos rfl, Fintype.sum_eq_single i]
      · rw [edge_z0]; simp
      · intro i' hi'
  ...
  have h2 : ∑ i', ∑ j, (s i' * ρ j * truncSq (t i') (x j)) *
    ...
    · rw [if_neg hih]
      ...
      have hne : idxZ (p := p) i ≠ idxZ h := fun eq => hih (idxZ_injective eq)
      rw [edge_zy]
      simp [hne, idxZ_ne_idxY (p := p) i j, idxZ_ne_idxY (p := p) h j]
      exact fun h1 h2 => (hih (h2.trans h1.symm)).elim
  rw [h1, h2]
  split_ifs with hih
...
lemma configL_z_y (i : Fin k) (j : Fin p) :
  ...
  have h1 : ∑ i', s i' *
    ...
    refine Fintype.sum_eq_zero _ fun i' => ?_
    rw [edge_z0]
    simp [idxZ_ne_idxY (p := p) i j, idxZ0_ne_idxY (k := k) j]
  have h2 : ∑ i', ∑ j', (s i' * ρ j' * truncSq (t i') (x j')) *
      vecMulVec (e (idxZ (p := p) i') - e (idxY (k := k) j'))
...
lemma configL_y_y (j ℓ : Fin p) :
  ...
  have h1 : ∑ i, s i *
    ...
    rw [edge_z0]
    simp [idxZ0_ne_idxY (k := k) j, idxZ_ne_idxY (p := p) i j,
      idxZ0_ne_idxY (k := k) ℓ, idxZ_ne_idxY (p := p) i ℓ]
  have h2 : ∑ i, ∑ j', (s i * ρ j' * truncSq (t i) (x j')) *
      vecMulVec (e (idxZ (p := p) i) - e (idxY (k := k) j'))
...
lemma MX_y_y_diag (hs : ∀ i, 0 < s i) (ht : ∀ i, 0 < t i) (hρ : ∀ j, 0 < ρ j)
    (hx : ∀ j, 0 ≤ x j) (j : Fin p) :
...
lemma M_eq_configF_mul_transpose_add_configL (hs : ∀ i, 0 < s i)
  ...
  · intro j
    ...
    · intro ℓ
      ...
      · rw [MX_y_y_diag s t ρ x hs ht hρ hx, FF_y_y, configL_y_y]
        simp
      · rw [MX_y_y_of_ne s t ρ x hρ hx hjl, FF_y_y, configL_y_y, if_neg hjl]
        ring
...
def configLEE_invDiag : Matrix (Option (Fin k)) (Option (Fin k)) ℝ :=
  diagonal fun
    | none => 0
...
lemma configLEE_mulVec_w (_hs : ∀ i, 0 < s i) (hρ : ∀ j, 0 < ρ j) :
  ...
  | some i =>
    ...
    simp only [configW, Option.some.injEq]
    have hd : configD t ρ x i ≠ 0 := (configD_pos t hρ x i).ne'
...
lemma configLEE_mul_invDiag_apply (a b : Option (Fin k)) :
    (configLEE s t ρ x * configLEE_invDiag s t ρ x) a b =
      match b with
      | none => 0
      | some j => configLEE s t ρ x a (some j) * (s j * configD t ρ x j)⁻¹ := by
  cases b with
  | none =>
    simp only [mul_apply, configLEE_invDiag]
...
lemma configLEE_inv (hs : ∀ i, 0 < s i) (hρ : ∀ j, 0 < ρ j)
  ...
  have hprod :
      configLEE s t ρ x *
        ...
        configLEE s t ρ x * configLEE_invDiag s t ρ x +
          (configγ s t ρ x)⁻¹ •
  ...
  | none =>
    cases b with
    | none =>
      have hL : (configLEE s t ρ x * configLEE_invDiag s t ρ x) none none = 0 :=
      ...
      field_simp [hγne]
    | some j =>
      have hL : (configLEE s t ρ x * configLEE_invDiag s t ρ x) none (some j) =
  ...
  | some i =>
    cases b with
    | none =>
      have hL : (configLEE s t ρ x * configLEE_invDiag s t ρ x) (some i) none = 0 :=
      ...
      simp [Matrix.add_apply, hL, hR, one_apply]
    | some j =>
      have hL : (configLEE s t ρ x * configLEE_invDiag s t ρ x) (some i) (some j) =
...
lemma configFE_transpose_mulVec_w (hρ : ∀ j, 0 < ρ j) :
...
lemma configLEE_invDiag_mul_FE_none :
    (configLEE_invDiag s t ρ x * configFE s t ρ x) none = (0 : Fin 3 → ℝ) := by
  ext c
  simp only [mul_apply, configLEE_invDiag, Pi.zero_apply]
  refine Fintype.sum_eq_zero _ fun a => ?_
  cases a <;> simp [diagonal]

lemma configLEE_invDiag_mul_FE_some (hs : ∀ i, 0 < s i) (hρ : ∀ j, 0 < ρ j)
    (i : Fin k) :
    (configLEE_invDiag s t ρ x * configFE s t ρ x) (some i) =
      (configD t ρ x i)⁻¹ • v (t i) := by
  ext c
  simp only [mul_apply, configLEE_invDiag, Pi.smul_apply, smul_eq_mul]
  rw [Fintype.sum_eq_single (some i)]
  · have hsi : s i ≠ 0 := (hs i).ne'
    have hd : configD t ρ x i ≠ 0 := (configD_pos t hρ x i).ne'
    simp only [configLEE_invDiag, diagonal_apply_eq, configFE_some, smul_eq_mul]
    have : (s i * configD t ρ x i)⁻¹ * (s i * v (t i) c) =
        (configD t ρ x i)⁻¹ * v (t i) c := by
      field_simp [hsi, hd]
    exact this
  · intro a ha
    have : configLEE_invDiag s t ρ x (some i) a = 0 := by
      simp only [configLEE_invDiag]
      exact diagonal_apply_ne _ ha.symm
    simp only [configLEE_invDiag] at this
    rw [this, zero_mul]

lemma vecMulVec_w_mul_FE (hρ : ∀ j, 0 < ρ j) (a : Option (Fin k)) :
  ...
  have hdot : ∑ b, configW t ρ x b * configFE s t ρ x b c =
    ...
    have := congrArg (fun f => f c) (configFE_transpose_mulVec_w s t ρ x hρ)
...
lemma configInvFE_none (hs : ∀ i, 0 < s i) (hρ : ∀ j, 0 < ρ j)
  ...
  have hW : ∑ a, configW t ρ x none * (configW t ρ x a * configFE s t ρ x a c) =
      Vvec t (configQ s t ρ x) c := by
    have := congrArg (fun f => f c) (vecMulVec_w_mul_FE s t ρ x hρ none)
...
lemma configInvFE_some (hs : ∀ i, 0 < s i) (hρ : ∀ j, 0 < ρ j)
  ...
  simp only [mul_apply, Matrix.add_apply, Matrix.smul_apply, vecMulVec_apply,
    smul_eq_mul]
  have hterm (a : Option (Fin k)) :
      (configLEE_invDiag s t ρ x (some i) a +
          (configγ s t ρ x)⁻¹ * (configW t ρ x (some i) * configW t ρ x a)) *
        configFE s t ρ x a c =
        configLEE_invDiag s t ρ x (some i) a * configFE s t ρ x a c +
          (configγ s t ρ x)⁻¹ *
            (configW t ρ x (some i) * (configW t ρ x a * configFE s t ρ x a c)) := by
    ring
  simp_rw [hterm, sum_add_distrib]
  have hD : ∑ a, configLEE_invDiag s t ρ x (some i) a * configFE s t ρ x a c =
  ...
  have hW : ∑ a, configW t ρ x (some i) * (configW t ρ x a * configFE s t ρ x a c) =
    ...
    have := congrArg (fun f => f c) (vecMulVec_w_mul_FE s t ρ x hρ (some i))
      ...
      -ρ j •
        (∑ i, (configQ s t ρ x i * truncSq (t i) (x j)) • v (t i) +
          (h t (configQ s t ρ x) (x j) / configγ s t ρ x) •
            Vvec t (configQ s t ρ x)) := by
  ext c
  simp only [mul_apply, Pi.add_apply, Pi.smul_apply, Pi.neg_apply, smul_eq_mul]
  rw [Fintype.sum_option]
...
lemma configU_apply (hs : ∀ i, 0 < s i) (hρ : ∀ j, 0 < ρ j)
  ...
  simp [U, bhat, Pi.add_apply, Pi.smul_apply, Pi.neg_apply, Finset.sum_apply,
    smul_eq_mul]
  ring

/-- Expand `(A * wwᵀ * C) j ℓ` without unfolding the index type of `w`. -/
lemma mul_vecMulVec_mul_apply {m n : Type*} [Fintype m] [Fintype n]
  ...
  have hsplit :
      configLTE s t ρ x *
          (configLEE_invDiag s t ρ x +
            (configγ s t ρ x)⁻¹ • vecMulVec (configW t ρ x) (configW t ρ x)) *
          configLET s t ρ x =
        configLTE s t ρ x * configLEE_invDiag s t ρ x * configLET s t ρ x +
          configLTE s t ρ x *
            ((configγ s t ρ x)⁻¹ • vecMulVec (configW t ρ x) (configW t ρ x)) *
            configLET s t ρ x := by
    rw [Matrix.mul_add, Matrix.add_mul]
  have hD :
      (configLTE s t ρ x * configLEE_invDiag s t ρ x * configLET s t ρ x) j ℓ =
        ρ j * ρ ℓ *
          ∑ i, configQ s t ρ x i * truncSq (t i) (x j) * truncSq (t i) (x ℓ) := by
    have hinner (a : Option (Fin k)) :
        ∑ b, configLTE s t ρ x j b * configLEE_invDiag s t ρ x b a =
          configLTE s t ρ x j a * configLEE_invDiag s t ρ x a a := by
      rw [Fintype.sum_eq_single a]
      · intro b hb
        simp only [configLEE_invDiag]
        rw [diagonal_apply_ne _ hb, mul_zero]
    simp only [mul_apply]
    have : ∑ a, (∑ b, configLTE s t ρ x j b * configLEE_invDiag s t ρ x b a) *
        configLET s t ρ x a ℓ =
        ∑ a, configLTE s t ρ x j a * configLEE_invDiag s t ρ x a a *
          configLET s t ρ x a ℓ := by
      refine Fintype.sum_congr _ _ fun a => ?_
      rw [hinner]
    rw [this, Fintype.sum_option]
    simp only [configLTE_none, configLET_none, configLEE_invDiag, diagonal_apply,
      zero_mul, mul_zero, zero_add]
    simp only [configLTE_some, configLET_some, configQ, div_eq_mul_inv,
      configLEE_invDiag, diagonal_apply_eq, ↓reduceIte]
    have hterm (i : Fin k) :
...
lemma configLEE_mulVec_one_add (a : Option (Fin k)) :
  ...
  cases a <;> simp [configEEmbed, add_comm, add_left_comm]
...
lemma elimEE_eq_LEE_add_FE (hs : ∀ i, 0 < s i) (ht : ∀ i, 0 < t i)
  ...
  rw [hM]
  simp [configLEE, configFE, Matrix.add_apply, submatrix_apply, mul_apply,
    transpose_apply, elimEEmbed, configEEmbed]
  exact add_comm _ _
...
lemma elimET_eq_LET_add_FE_FT (hs : ∀ i, 0 < s i) (ht : ∀ i, 0 < t i)
  ...
  rw [hM]
  simp [configLET, configFE, configFT, Matrix.add_apply, submatrix_apply,
    mul_apply, transpose_apply, elimEEmbed, elimT, configEEmbed]
  exact add_comm _ _
...
lemma elimTE_eq_LTE_add_FT_FE (hs : ∀ i, 0 < s i) (ht : ∀ i, 0 < t i)
  ...
  rw [hM]
  simp [configLTE, configFE, configFT, Matrix.add_apply, submatrix_apply,
    mul_apply, transpose_apply, elimEEmbed, elimT, configEEmbed]
  exact add_comm _ _
...
lemma configLEE_inv_transpose (hs : ∀ i, 0 < s i) (hρ : ∀ j, 0 < ρ j)
    (hγ : 0 < configγ s t ρ x) :
...
lemma configET_elim_factor (hs : ∀ i, 0 < s i) (ht : ∀ i, 0 < t i)
  ...
  rw [configLEE_inv_transpose s t ρ x hs hρ hγ, configLET_eq_transpose]
...
lemma schur_prod_expand {ε τ κ : Type*} [Fintype ε] [Fintype τ] [Fintype κ]
    [DecidableEq ε]
...
lemma elimSchurR_eq_woodbury (hs : ∀ i, 0 < s i) (ht : ∀ i, 0 < t i)
  ...
  have hTT : elimTT s t ρ x =
    ...
    simp only [Z, Z', U, transpose_add, transpose_mul, Matrix.add_mul,
      Matrix.mul_add, Matrix.mul_assoc, configLEE_inv_transpose s t ρ x hs hρ hγ,
...
lemma isCompletelyPositive_mul_conj_diagonal {n : Type*} [Fintype n] [DecidableEq n]
...
lemma isCompletelyPositive_of_submatrix_pad {ι n : Type*}
    [Fintype ι] [DecidableEq ι] [Fintype n] [DecidableEq n]
...
lemma configLredWeights_nonneg (hs : ∀ i, 0 < s i) (hρ : ∀ j, 0 < ρ j)
  ...
  · intro j hlt
    ...
    · intro ℓ h
      ...
      simp [configLredWeights_y_y, hjℓ]
...
/- Pair corollary inlined: `BN.CP.Pair` clashes with `BN.M.Basic` on
  ...
  have hsum : ∑ p : n × n, (if p.1 < p.2 then f p else 0) =
    ...
    have hite (p : n × n) :
      ...
      · have hp : p ∉ offDiagLt := by
        ...
        simp [hlt, hp]
    simp_rw [hite]
    simpa using (Finset.sum_ite_mem (Finset.univ : Finset (n × n)) offDiagLt f).symm
...
lemma extendByZeroT_mulVec_one (A : Matrix (Fin p) (Fin p) ℝ)
  ...
  · intro j
    ...
    rw [sum_configIdx]
    simp [extendByZeroT_z0_right, extendByZeroT_z_right, extendByZeroT_y_y]
    exact hrow
