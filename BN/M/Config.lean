...
lemma MX_z0_z0 (hs : ∀ i, 0 < s i) (hx : ∀ j, 0 ≤ x j) :
  ...
  have hsum :
    ...
    have hz : ∑ i : Fin k,
      ...
      rw [if_pos (And.intro hlt' hneg), Xconfig_z0_z, neg_sq, Real.sq_sqrt (hs i).le]
    have hy : ∑ j : Fin p,
  ...
  have hup :
    ...
    have hy : ∑ j : Fin p,
      ...
      · have hneg : Real.sqrt (s i) * Real.sqrt (ρ j) * (t i - x j) < 0 :=
        ...
        simp [hneg, hmax, mul_pow, Real.sq_sqrt (hs i).le, Real.sq_sqrt (hρ j).le]
