...
lemma exists_nonneg_gram_factor (z : n → Fin 2 → ℝ)
  ...
  · push Not at hz
    ...
    have hα0 : 0 ≤ α 0 := by
      simp [α, normalize, Pi.smul_apply, smul_eq_mul]
    ...
    · intro i
      ...
      · have hwi : w i ≠ 0 := mt (rotateTo_eq_zero_iff u (z i) hu).mp hzi
        ...
        have hγ0 : 0 ≤ normalize (w i) 0 := by
          simp [normalize, Pi.smul_apply, smul_eq_mul]
...
lemma sum_comp_injective {ι : Type*} [Fintype ι] [DecidableEq ι]
...
lemma M_eq_zero_of_not_mem_range {ι : Type*} [Fintype ι]
...
lemma M_eq_zero_of_not_mem_range_right {ι : Type*} [Fintype ι]
    {X : Matrix n n ℝ} (hX : X.IsSymm) {e : ι → n}
    (hzero : ∀ i, i ∉ Set.range e → ∀ j, X i j = 0)
...
lemma det2_perturbTied {z : n → Fin 2 → ℝ} {i0 : n} {v : Fin 2 → ℝ}
  ...
  · rw [hzi0]
    ...
    · subst hi
      rw [perturbTied_i0]
      simp [det2, normalize, smul_eq_mul]
