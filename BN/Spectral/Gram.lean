...
`BN.Spectral.Variational` cannot be imported here: it shares `inner_vecMulVec`
with `BN.MS.Basic` (pulled in by `Weighted`), and also overlaps Weighted on
...
lemma vecMulVec_mulVec_real (u v x : n → ℝ) :
  ...
  ext i
  simp [mulVec, vecMulVec_apply, dotProduct, smul_eq_mul]
  trans u i * ∑ j, v j * x j
  · simp_rw [mul_assoc]
...
/-- **CG05.** Theorem `thm:gram`. Named `gram_le` because `BN.gram` is already
the planar Gram matrix in `BN.M.HalfPlane`. -/
