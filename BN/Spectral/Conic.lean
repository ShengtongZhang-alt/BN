...
lemma rank_smul_le (c : ℝ) (A : Matrix V V ℝ) : (c • A).rank ≤ A.rank := by
  refine finrank_mono ?_
  rintro _ ⟨x, rfl⟩
  refine ⟨c • x, ?_⟩
  ext i
  simp [mulVec, dotProduct, smul_eq_mul]
...
lemma inner_adjMatrix_hadamard_self_eq_sum_posPart_sq {X : Matrix V V ℝ}
  ...
  by_cases hij : G.Adj i j
  · rw [if_pos hij, posPart_eq_of_nonneg X (hnn i j hij), pow_two]
  · rw [if_neg hij, zero_mul, zero_mul]
