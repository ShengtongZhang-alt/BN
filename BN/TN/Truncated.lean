...
lemma truncatedSquare_det_three_mixed {a t : Fin 3 → ℝ}
  ...
  have hpoly :
      (truncatedSquare a t).det =
        4 * x * u * v * r * q ^ 2 + 4 * x * u * v * r ^ 2 * q +
          2 * x * u * v ^ 2 * q ^ 2 + 4 * x * u * v ^ 2 * r * q +
          2 * x * u ^ 2 * r * q ^ 2 + 2 * x * u ^ 2 * r ^ 2 * q +
          2 * x * u ^ 2 * v * q ^ 2 + 4 * x * u ^ 2 * v * r * q +
          2 * x ^ 2 * v * r * q ^ 2 + 2 * x ^ 2 * v * r ^ 2 * q +
          x ^ 2 * v ^ 2 * q ^ 2 + 2 * x ^ 2 * v ^ 2 * r * q +
          2 * x ^ 2 * u * r * q ^ 2 + 2 * x ^ 2 * u * r ^ 2 * q +
          2 * x ^ 2 * u * v * q ^ 2 + 4 * x ^ 2 * u * v * r * q := by
    rw [det_fin_three]
...
noncomputable instance (k m : ℕ) : DecidablePred fun f : Fin k → Fin m => StrictMono f :=
  Classical.decPred _
...
lemma gridK_apply {r c : ℕ} (hr : 0 < r) (a : Fin r → ℝ) (t : Fin c → ℝ)
  ...
  have h1 : ∀ k,
    ...
    · change ∑ x ∈ (univ : Finset (Fin (N + 1))),
          ite (x = k) (gridH1 hr a t N i k * δ) 0 = _
      simp [sum_ite_eq]
  have h2 : ∀ p,
  ...
  have ksplit :
      ∀ k, (if k ∈ gridSupport hr a t N i j ∧ k ≤ ℓ then (1 : ℝ) else 0) =
        if k ∈ gridSupport hr a t N i j then
          (if k ≤ ℓ then (1 : ℝ) else 0) else 0 := by
    intro k
    split_ifs <;> simp_all
  simp_rw [ksplit]
  rw [← sum_filter, filter_univ_mem]
...
lemma gridSupport_card_mul_delta_sub_le {r c : ℕ} (hr : 0 < r)
  ...
  · have hinter := gridSupport_interval hr a t hN i j hne
    ...
    have hup : (S.card : ℝ) * δ - L ≤ δ := by
      rw [hmd, hL]
      nlinarith [hxℓmax.2, hxkmin.1]
    have hlow : L - (S.card : ℝ) * δ ≤ 2 * δ := by
      rw [hmd, hL]
      nlinarith [hxkpred, hxℓsucc, hxkmin_eq, hxℓmax_eq]
    rw [abs_le]
  ...
  · have hm0 : S.card = 0 := card_eq_zero.mpr (not_nonempty_iff_eq_empty.mp hne)
    ...
    · have hlt : a i < t j := not_le.mp hta
      ...
      have hknext_gt : kstar < knext :=
        Fin.lt_def.mpr (by change kstar.val < knext.val; simp [knext])
