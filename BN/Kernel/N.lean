...
lemma det_D (x : ℝ) : (D x).det = 1 := by
  rw [det_fin_three]
  simp [D, fromThreeCols, b, Pi.single_apply]
...
lemma updateCol_two_eq_fromThreeCols (A : Matrix (Fin 3) (Fin 3) ℝ) (w : Fin 3 → ℝ) :
  ...
  rw [updateCol_apply]
  fin_cases j
  · simp [fromThreeCols, mulVec_single_one, col_apply]
  · simp [fromThreeCols, mulVec_single_one, col_apply]
  · simp [fromThreeCols]
...
lemma fromThreeCols_eq_D_add_sum (x : ℝ) :
  ...
  have hsum :
    ...
    ext a b
    fin_cases b <;>
      simp [fromThreeCols, Matrix.sum_apply, Matrix.smul_apply, vecMulVec_apply, cVec,
...
lemma fKernel_convexOn {x : ℝ} (hx : 0 ≤ x) :
...
lemma isTotallyNonneg_fKernelRows (x : ℝ) (hx : 0 ≤ x)
  ...
  isTotallyNonneg_convexRowMatrix (fKernel_convexOn hx) (fKernel_zero hx)
...
lemma sqRows_eq_scale :
  ...
  fin_cases j <;> simp [row3, cons_val_zero, cons_val_one, cons_val_two, vecHead, vecTail] <;>
    ring
...
lemma IsTotallyNonneg.det_principal {n : Type*} [LinearOrder n] [Fintype n]
