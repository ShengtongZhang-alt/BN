...
lemma inner_vecMulVec_sub_single [Fintype n] [DecidableEq n]
  ...
  have hxj : x j = -1 := by
    simp only [x]
    rw [sub_single_apply hij, if_neg hij.symm, if_pos rfl]
...
theorem posSemidef_hadamard [Fintype n] [DecidableEq n]
...
theorem posSemidef_hadamard_self [Fintype n] [DecidableEq n]
