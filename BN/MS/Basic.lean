...
lemma adjMatrix_quadratic_add_smul_single_sub_single
  ...
  have hj : 0 ≤ (y + t • (e i - e j)) j := by
    rw [add_smul_e_sub_e_apply hne, if_neg hne.symm, if_pos rfl]
    exact htj
  have _ := hi
  have _ := hj
  have hmul :
...
lemma stdSimplex_nonempty [Nonempty V] : (stdSimplex ℝ V).Nonempty :=
    ...
    | h n ih =>
      ...
      · obtain ⟨⟨i, hi⟩, ⟨j, hj⟩, hne₀, hnad⟩ :=
        ...
        have ht_mul : 0 ≤ t * δ := by
          by_cases hδ : 0 ≤ δ
          · rw [show t = y j from if_pos hδ]
            exact mul_nonneg hyj.le hδ
          · rw [show t = -y i from if_neg hδ]
        ...
        have hyj' : (y + t • (e i - e j)) j = 0 := by
          rw [add_smul_e_sub_e_apply hne, if_neg hne.symm, if_pos rfl, htj0]
          ring
...
lemma adjMatrix_quadratic_eq_sum_sq_sub_of_isClique
  ...
  have hterm : ∀ i j,
    ...
    · have hne : i ≠ j := hAdj.ne
      rw [if_pos hAdj, if_neg hne]
      ring
    · by_cases hij : i = j
      · subst hij
        rw [if_neg hAdj, if_pos rfl]
        ring
      · rw [if_neg hAdj, if_neg hij, hprod i j hAdj hij]
        ring
